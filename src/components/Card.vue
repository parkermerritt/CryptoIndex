...
/* eslint-disable */
  ...
  computed: {
    cardDim: function () {
      ...
      var thisFlipper = $('.card__flipper')
      //  var offset = self.offset()
      var offL = this.offsetLeft
      var offT = this.offsetTop

      var xc = win.width() / 2
      var yc = win.height() / 2
      var docScroll = $(document).scrollTop()
      var cardW = self.outerWidth / 2
      var cardH = self.height / 2

      //  var transX = xc - offset.left - cardW
      //  var transY = docScroll + yc - offset.top - cardH
      var transX = xc - offL - cardW
      var transY = docScroll + yc - offT - cardH
  ...
  methods: {
    ...
    //  var self = this
    //  var card = $('.card')
    //  var flipper = self.find('.card__flipper')
    //  var win = $(window)
    //  },

    unflip: function (event) {
      var flipper = $('.card__flipper')
    ...
    flip: function (event) {
      ...
      // if (this.hasClass('active')) unflip()

      //  self.css({'z-index': '3'}).addClass('active')
      this.css({'z-index': '3'}).addClass('active')
...
<!-- End of Card template Javascript-->
